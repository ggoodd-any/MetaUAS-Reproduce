...
class InpatinedCocoDataset(Dataset):
    def __init__(self, path_to_dataset, split, method, image_transformation, machine="local"):
        ...
        self.marshal_getitem_data = self.import_method_specific_functions()
        self.dtd_dir = r'/home/ggoodd/code/qh/Dataset/DTD/images'
        self.dtd_paths = glob.glob(os.path.join(self.dtd_dir, "**/*.jpg"),
                                   recursive=True)

    def import_method_specific_functions(self):
        from metauas_wrapper import marshal_getitem_data
    ...
    def get_train_val_test_split(self, split):
        ...
        # train_val_test_split_file_path = os.path.join(self.path_to_dataset, "data_split_10_percent.pkl")
    ...
    def add_random_perlin_noise(self, image_as_tensor, item_index):
        perlin_noise = perlin.get_perlin_noise(image_as_tensor.shape[-2:])
        texture_image = self.read_image_as_tensor(random.choice(self.dtd_paths))
        noise_strength = random.uniform(0.3, 1)
        image_as_tensor, perlin_noise_mask, texture_image = perlin.image_blender_with_perlin_noise(
            image_as_tensor, texture_image, perlin_noise,
            1 - noise_strength, noise_strength)

        annotations = perlin.convert_mask_to_coco_annotations(perlin_noise_mask)

        (
            texture_image_resized_to_current,
            annotations_resized,
        ) = utils.geometry.resize_image_and_annotations(
            texture_image, image_as_tensor.shape[-2:], annotations
        )

        return image_as_tensor, annotations_resized
    ...
    def __base_getitem__(self, item_index):
        ...
        if random.random() < 0.5 or self.split == "test":
            ...
            if random.random() < 0.5:
                image2_image_as_tensor, annotations = self.add_random_objects(image2_image_as_tensor, item_index)
            else:
                image2_image_as_tensor, annotations = self.add_random_perlin_noise(image2_image_as_tensor, item_index)

            if random.random() < 0.5:
                image1_image_as_tensor, image2_image_as_tensor = image2_image_as_tensor,image1_image_as_tensor
