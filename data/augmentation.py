...
class AugmentationPipeline(nn.Module):
    def __init__(self, mode, path_to_dataset, image_transformation) -> None:
        ...
        self.norm = K.augmentation.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        self.mode = mode
        self.path_to_dataset = path_to_dataset
        self.image_transformation = image_transformation


    def set_augmentation_level(self, level: int):
        # do not thing
        pass

        # if level == 0:
        #     self.jit = K.augmentation.ColorJitter(0.1, 0.0, 0.0, 0.0, p=0.0, keepdim=True)
        #     self.aff = K.augmentation.RandomAffine(
        #         degrees=30,
        #         translate=(0.2, 0.2),
        #         scale=(0.8, 1.5, 0.8, 1.5),
